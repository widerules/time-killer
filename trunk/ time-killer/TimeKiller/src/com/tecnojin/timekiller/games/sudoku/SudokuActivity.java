...
public class SudokuActivity extends GameActivity{
	...
	private SudokuState resolveState() {
		...
		try {
			int dif=Integer.parseInt(GameManager.instance(this).getGame(GameManager.SUDOKU, this).getOptions().findOptionForKey("diff").getCurrentValue());
			...
			Integer min=Integer.parseInt(m.get(0).get("min(id)").toString());
			Integer max=Integer.parseInt(M.get(0).get("max(id)").toString());
			
			int id=MathUtil.random(min, max);
